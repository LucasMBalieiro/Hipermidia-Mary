...
public class GameManager : MonoBehaviour
{
    ...
    public List<GestureTemplate> gestureTemplate;
    ...
    private void InitializeGestureTemplates()
    {
        ...
        {
            gestureTemplate.Add(PennyPincher.CreateTemplate(template.name, template.points));
            template.points.Reverse();
            gestureTemplate.Add(PennyPincher.CreateTemplate(template.name, template.points));
        }
        
        Debug.Log("Template Count: " + templateScriptableObjects.Length);
    }
}
