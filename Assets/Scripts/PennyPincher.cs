...
public static class PennyPincher
{
    ...
    public static RecognitionResult Recognize(List<Vector2> points)
    {
        ...
        return bestMatch != null ? new RecognitionResult { name = bestMatch.name, score = bestScore } : RecognitionResult.NoMatch;
    }
    
    public static GestureTemplate CreateTemplate(string name, List<Vector2> points)
    {
        List<Vector2> resampledPoints = Resample(points, NumResamplePoints);
        Vector2[] normalizedVectors = Vectorize(resampledPoints, true); 
        return new GestureTemplate(name, normalizedVectors);
    }
